...
contract ConvictionVotingCreditDelegation is TimeHelpers {
    ...
    mapping(address => uint256) internal totalAccountDeposited;
    mapping(address => uint256) internal totalAccountBorrowed;
    ...
    function depositCollateral(uint256 amount) public {
        // Deposit code
        totalAccountDeposited[msg.sender] = totalAccountDeposited[msg.sender].add(amount);
		totalDeposited = totalDeposited.add(amount);
    }

/*    function canBorrow(address projectAddress) public view returns(bool) {
		// TODO: improve voting system
		return projectsMap.projects[projectAddress].voteCount > 67;
	}*/

    function borrowCredit(uint256 amount, address asset) public {
        require(checkAllowance(msg.sender, asset) >= amount, "Cannot Borrow more than what allowed");
        // Borrow credit
        totalAccountBorrowed[msg.sender] = totalAccountBorrowed[msg.sender].add(amount);
    }

    function checkAllowance(address borrower, address asset) public view returns(uint256) {}

    function repayBorrowing(uint256 amount) public {
        // TODO: complete
        totalAccountBorrowed[msg.sender] = totalAccountBorrowed[msg.sender].sub(amount);
    }

    /*    function withdrawCollateral(address asset) public {
        // TODO: complete
        uint assetBalance = 1;
        totalAccountDeposited[msg.sender] = totalAccountDeposited[msg.sender].sub(assetBalance);
		totalAccountDeposited = totalAccountDeposited.sub(assetBalance);
    }*/
...
/*    function stakeAllToProposal(uint256 _proposalId) external {
    ...
    function executeProposal(uint256 _proposalId) external proposalExists(_proposalId) {
        ...
        // TODO: replace : vault.transfer(requestToken, proposal.beneficiary, proposal.requestedAmount);
        // by allow borrower on Aave
        _approveBorrower(_proposalId);
    ...
    function calculateThreshold(uint256 _requestedAmount) public view returns (uint256 _threshold) {
        require(maxRatio.mul(totalDeposited) > _requestedAmount.mul(D), ERROR_AMOUNT_OVER_MAX_RATIO);
        // denom = maxRatio * 2 ** 64 / D  - requestedAmount * 2 ** 64 / funds
        uint256 denom = (maxRatio << 64).div(D).sub((_requestedAmount << 64).div(totalDeposited));
    ...
    function _approveBorrower(uint256 _proposalId) internal {
        // approveBorrower
    }

    function _totalStaked() internal view returns (uint256) {
        uint256 minTotalStake = (totalDeposited.mul(minThresholdStakePercentage)).div(ONE_HUNDRED_PERCENT);
    ...
    function _stake(uint256 _proposalId, uint256 _amount, address _from) internal proposalExists(_proposalId) {
        ...
        uint256 unstakedAmount = totalAccountDeposited[_from].sub(totalVoterStake[_from]);
